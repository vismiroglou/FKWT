...
def main(batch, fourier_seq, attention_seq):
    #end = 0
    #batch = torch.rand([int(batch_size), int(seq_len), int(embed_len)])
    #device = torch.device('cuda')
    #fourier_seq = fourier_seq.to(device)
    #attention_seq = attention_seq.to(device)5
    ...
    fourier_time = sum(fourier_times)/len(fourier_times)*1000
    attention_time = sum(attention_times)/len(attention_times)*1000
...
if __name__ == '__main__':
    ...
    num_heads = [1,2,3]
    mlp_ratio = 4
    for embed_dim, num_heads in tqdm(zip(embed_dims, num_heads), position=0):
        #P_Norm = PreNorm
        P_Norm = PostNorm
